...
from utils.tool_execution_logger import log_tool_execution, logged_tool
...
async def agent_review_endpoint(request: Request) -> JSONResponse:
    ...
    try:
        ...
        log_tool_execution(
            tool_name="agent_review_endpoint",
    ...
    if not repo_name or pr_number is None:
        log_tool_execution(
            tool_name="agent_review_endpoint",
            kwargs={"repo_name": repo_name, "pr_number": pr_number},
            result={"ok": False, "error": "repo_name and pr_number are required"},
            duration_ms=round((time.time() - started_at) * 1000, 2),
        )
        return JSONResponse(
    ...
    try:
        ...
        log_tool_execution(
            tool_name="agent_review_endpoint",
            kwargs={"repo_name": repo_name, "pr_number": pr_number},
            result={"ok": False, "error": "pr_number must be an integer"},
            duration_ms=round((time.time() - started_at) * 1000, 2),
        )
    ...
    if not context.get("ok"):
        log_tool_execution(
            tool_name="agent_review_endpoint",
            kwargs={"repo_name": repo_name, "pr_number": pr_number},
            result={"ok": False, "error": context.get("error", "Unable to collect PR review context")},
            duration_ms=round((time.time() - started_at) * 1000, 2),
        )
        return JSONResponse(
            {
